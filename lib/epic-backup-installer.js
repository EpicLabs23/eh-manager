...
export const performEpicBackupInstall = async ({ version, apiPublicUrl, targetDir }) => {
  ...
  try {
    const url = `http://release.epiclabs23.com/epic-backup-api-${version}.tar.gz`;
    ...
    const downloadPath = path.join(extractDir, `epic-backup-api-${version}.tar.gz`);
    ...
    // Run install
    execSync(`npm install --omit=dev`, { stdio: 'inherit', cwd: extractDir });
    execSync(`npx prisma db push --schema=./dist/prisma/schema.prisma`, { stdio: 'inherit', cwd: extractDir });

    // Clean up
    fs.rmSync(downloadPath);
    fs.rmSync(path.join(extractDir, 'dist/prisma/schema.prisma'));
    ...
    if (epicBackupApiProcess) {
        execSync(`pm2 delete epic-backup-api`, { stdio: 'inherit', cwd: extractDir });
    }
    execSync(`pm2 start "node dist/main" --name epic-backup-api`, { stdio: 'inherit', cwd: extractDir });
