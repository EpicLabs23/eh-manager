...
export const performEpicBackupInstall = async ({ version, apiurl, targetDir }) => {
  ...
  try {
    const url = `https://github.com/${PUBLIC_RELEASE_REPO}/releases/download/${version}/epic-backup-${version}.tar.gz`;
    ...
    const envPath = path.join(extractDir, 'epic-backup-api', '.env');
