...
export const performEpicBackupUpdate = async ({ newVersion, apiurl, targetDir }) => {
  ...
  try {
    const url = `http://release.epiclabs23.com/epic-backup-${newVersion}.tar.gz`;
    ...
    fs.copyFileSync(path.join(targetDir, 'temp', 'dev.db'), path.join(apiPath, 'dist/prisma/dev.db'));
