...
export const performEhmUpdate = async ({
  ...
  try {
    ...
    if (fs.existsSync(installScript)) {
      ...
      // Quote every arg: apiPublicUrl (and others) can be an empty string when
      // carried forward with nothing to carry, and an unquoted empty arg vanishes
      // in shell word-splitting, shifting every positional argument after it.
      const args = [newVersion, mysqlRootPassword, apiPublicUrl, influxEnabled, encryptionKey, influxToken, hostMetricsEnabled]
